...
import org.swarg.mcforge.util.CustomTeleporter;
...
public class AdminToolsCommands extends CommandBase
{
    ...
    {
        return "/atools <mobclear/chestclear/restoreplayer/chunkregen/mobfind/findte/findblock/entityinfo/tileentityinfo/iteminfo/automessage/player/username-cache/item-scanner/get-dimension/jump2dim>";
    }
        ...
        if (!world.isRemote) {
            if (argString.length == 0) {
                sender.addChatMessage(new ChatComponentText("/atools <mobclear (mob, range) / chestclear (range) / restoreplayer / chunkregen / mobfind (mob, range) / findte (te, range)  / findblock (block, range)  / entityinfo (range) / tileentityinfo (range) / iteminfo / automessage (reload)"+
                        " / player / username-cache / item-scanner"
...
//                IChunkProvider chunkProviderGenerate = ObfuscationReflectionHelper.getPrivateValue(ChunkProviderServer.class, chunkProviderServer, "d", "field_73246_d");
            ...
            if (isCmd(cmd, "player", "p")) {
                org.swarg.mcforge.tools.PlayerCommands.instance().cmdPlayer(sender, argString);
            }
            else if (isCmd(cmd, "username-cache", "uc")) {
                cmdForgeUsernameUUIDCache(sender, argString);
            }
            else if (isCmd(cmd, "item-scanner", "isc")) {
                org.swarg.mcforge.tools.ItemScanWorker.instance().cmdItemScanner(sender, argString);
            }
     ...
     * узнать UUID-игрока для заданного имени игрока или наоборот
     * Например может быть полезно когда нужно отредактировать data-файл
     * конкретного игрока  world/playerdata/uuid.dat
     * O PlayerProfileCache:
     * MinecraftServer.getServer().func_152358_ax().func_152654_a();
     * PlayerProfileCache кэш актуален либо для лицензионных серверов либо для
     * серверов оффлайн режима без своей регистрации игроков
     * Данный метод работает на основе Forge:UsernameCache, данные о uuid-name
     * берутся в момент входа игрока на сервер. хранятся в файле "usernamecache.json"
     * и похоже никогда не очищаются.
     * @param sender
     * @param args
     * username-cache
     */
    public void cmdForgeUsernameUUIDCache(ICommandSender sender, String[] args) {
        int i = 1;
        final String cmd = arg(args, i++);
        String response = "?";
        if (isNullOrEmpty(cmd) || "help".equals(cmd)) {
            response = "<size/get-uuid/get-name/own>";
        }
        else if (isCmd(cmd, "size", "sz")) {
            response = "Forge:UsernameCache.size: " + XPlayer.getForgeUsernameCacheSize();
        }
        //own name: uuid
        else if (isCmd(cmd, "my", "own") && sender instanceof EntityPlayer) {
            response = sender.getCommandSenderName() + ": " + ((EntityPlayer)sender).getGameProfile().getId();
        }
        //name->uuid
        else if (isCmd(cmd, "get-uuid", "gu")) {
            final String name = arg(args, i++);
            if (isNullOrEmpty(name) || "help".equalsIgnoreCase(name)) {
                response = "(player-name)";
            } else {
                UUID uuid = XPlayer.getUUIDForPlayerNameFromForgeUsernameCache(name);
                response = "Forge:UsernameCache: " + uuid + " for " + name;
            }
        }
        //uuid -> name
        else if (isCmd(cmd, "get-name", "gn")) {
            final String sUuid = arg(args, i++);
            if (isNullOrEmpty(sUuid) || "help".equalsIgnoreCase(sUuid)) {
                response = "(str_playerUuid) | (long_mostSigBits) (long_leastSigBits)";
            } else {
                final int sz = args.length;
                UUID uuid = null;
                if (sz - i == 1) {
                    long mostSigBits = Long.parseLong(sUuid);
                    long leastSigBits = Long.parseLong(arg(args, i++));
                    uuid = new UUID(mostSigBits, leastSigBits);
                    toSender(sender, "Maked UUID of two long: " + uuid);
                } else {
                    try {
                        uuid = UUID.fromString(sUuid);
                    }
                    catch (Exception e) {
                        toSender(sender, "Illegal uuid");
                        return;
                    }
                }
                if (uuid != null) {
                    String name = net.minecraftforge.common.UsernameCache.getLastKnownUsername(uuid);
                    response = name +": "+ uuid;
                } else {
                    response = "illegal uuid";
                }
            }
        }
